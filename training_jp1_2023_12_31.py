        ...
        with gr.Tab('Train LoRA', elem_id='lora-train-tab'):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    gr.Markdown("大規模なデータセットの場合は数時間かかる場合がありますが、小規模な実行の場合はわずか数分しかかかりません")
                    ...
                    with gr.Row():
                        with gr.Column(scale=5):
                            lora_name = gr.Textbox(label='Name', info='新しい LoRA ファイルの名前')
                        with gr.Column():
                            always_override = gr.Checkbox(label='Override Existing Files', value=False, info='もし名前が同じなら、チェックを入れると既存のファイルを置き換え、チェックを外すとそのファイルをロードして続行する（ランクは同じでなければならない）', elem_classes=['no-background'])

                    with gr.Accordion(label='Target Modules', open=False):
                        gr.Markdown("トレーニングでターゲットにするモジュールを選択します。注意：model_id='llama'に対してのみ機能し、他のタイプではデフォルトのトレーニング動作が維持され、これらの設定は使用されません。")
                        with gr.Row():
                            with gr.Column():
                    ...
                    with gr.Row():    
                        gr.Markdown("次に、必要な学習量を考慮する必要があります")            

                    with gr.Row():
                        with gr.Column():
                            lora_rank = gr.Slider(label='LoRA Rank', value=32, minimum=0, maximum=1024, step=4, info='ディメンション・カウントとも呼ばれる。値が大きい＝ファイルが大きく、コンテンツのコントロールができる。値が小さい＝ファイルサイズが小さくなり、制御性が低下する。スタイルには4または8、ティーチングには128または256、ビッグデータの詳細には1024以上を使用する。より高いランクにはより多くのVRAMが必要です')
                            lora_alpha = gr.Slider(label='LoRA Alpha', value=64, minimum=0, maximum=2048, step=4, info='これをランクで割ったものがLoRAのスケーリングとなる。高ければ高いほど強い。目安はランクの2倍')
                            batch_size = gr.Slider(label='Batch Size', value=128, minimum=0, maximum=1024, step=4, info='グローバルバッチサイズ。2つのバッチサイズを合わせて、勾配累積を決定する（gradientAccum = batch / microBatch）。gradientAccumの値が大きいほど、より質の高いトレーニングになります')
                            micro_batch_size = gr.Slider(label='Micro Batch Size', value=4, minimum=1, maximum=128, step=1, info='デバイスごとのバッチサイズ（注：複数デバイスは未実装）。これを増やすとVRAM使用量が増えます')
                            cutoff_len = gr.Slider(label='Cutoff Length', minimum=0, maximum=4096, value=256, step=32, info='テキスト入力のカットオフ長。基本的に、一度に入力するテキスト行の長さ。値が大きいほど、より多くのVRAMを必要とする')

                        with gr.Column():
                            save_steps = gr.Number(label='Save every n steps', value=0, info='0以上の場合、LoRAのチェックポイントは、このステップ数が経過する度に保存される')

                            epochs = gr.Number(label='Epochs', value=3, info='データセットの各項目をトレーニングに投入する回数。つまり、1は各項目を1回ずつ入力することを意味し、5は5回入力することを意味する')
                            learning_rate = gr.Textbox(label='Learning Rate', value='3e-4', info='科学的表記法では 3e-4は良い出発点である。1e-2は極めて高く、1e-6は極めて低い')
                            with gr.Row():
                                lr_scheduler_type = gr.Dropdown(label='LR Scheduler', value='linear', choices=['linear', 'constant', 'constant_with_warmup', 'cosine', 'cosine_with_restarts', 'polynomial', 'inverse_sqrt'], info='学習率スケジューラー - 学習率が時間とともにどのように変化するかを定義する。「一定 "は変化しないことを意味し、"線形 "は学習率から0まで一直線に進むこと、余弦は曲線に従うことなどを意味する', elem_classes=['slim-dropdown'])

                    with gr.Accordion(label='高度なオプション', open=False):
                        with gr.Row():
                            with gr.Column():
                                lora_dropout = gr.Slider(label='LoRA Dropout', minimum=0.0, maximum=1.0, step=0.025, value=0.05, info='LoRA層の脱落確率。これはオーバーフィッティングを減らすのに役立つ。ほとんどのユーザーはデフォルトのままにしておくべきである')
                                stop_at_loss = gr.Slider(label='Stop at loss', minimum=0.0, maximum=3.0, step=0.1, value=0.00, info='希望する損失値に達すると、プロセスは自動的に停止する。(妥当な数値は1.5-1.8)')
                                with gr.Row():
                                    optimizer = gr.Dropdown(label='Optimizer', value='adamw_torch', choices=['adamw_hf', 'adamw_torch', 'adamw_torch_fused', 'adamw_torch_xla', 'adamw_apex_fused', 'adafactor', 'adamw_bnb_8bit', 'adamw_anyprecision', 'sgd', 'adagrad'], info='上級ユーザー向けの、さまざまなオプティマイザー実装オプション。さまざまなオプションの効果は、まだ十分に文書化されていません', elem_classes=['slim-dropdown'])

                            with gr.Column():
                                warmup_steps = gr.Number(label='Warmup Steps', value=100, info='開始時にこれだけのステップを踏むと、学習率は通常より低くなる。これは、トレーナーがモデルを準備し、開始後のトレーニングの質を向上させるための統計量を事前に計算するのに役立ちます')
                                train_only_after = gr.Textbox(label='Train Only After', value='', info='任意のチャンクで、この文字列以降のテキストのみを学習対象とする。Alpacaデータセットの場合、"### Response: "を使って応答だけを学習し、入力は無視する')

                                add_eos_token = gr.Checkbox(label='Add EOS token', value=False, info="各データセット項目にEOSトークンを追加する．生テキストの場合、EOSはHard Cutで追加される")

                                higher_rank_limit = gr.Checkbox(label='Enable higher ranks', value=False, info='チェックした場合、上のRank/Alphaスライダをより高く変更します。データセンタークラスのGPUがないと動作しません')
                                report_to = gr.Radio(label="Save detailed logs with", value="None", choices=["None", "wandb", "tensorboard"], interactive=True)

                with gr.Column():
                    with gr.Tab(label='Formatted Dataset'):
                        with gr.Row():
                            format = gr.Dropdown(choices=utils.get_datasets('training/formats', 'json'), value='None', label='Data Format', info='データセット入力をどのようにフォーマットするかを決定するためのフォーマット・ファイル', elem_classes=['slim-dropdown'], interactive=not mu)
                        ...
                        with gr.Row():
                            dataset = gr.Dropdown(choices=utils.get_datasets('training/datasets', 'json'), value='None', label='Dataset', info='トレーニングに使用するデータセットファイル', elem_classes=['slim-dropdown'], interactive=not mu)
                        ...
                        with gr.Row():
                            eval_dataset = gr.Dropdown(choices=utils.get_datasets('training/datasets', 'json'), value='None', label='Evaluation Dataset', info='(オプションの)データセットファイルは、トレーニング後にモデルを評価するために使用されます', elem_classes=['slim-dropdown'], interactive=not mu)
                        ...
                        eval_steps = gr.Number(label='Evaluate every n steps', value=100, info='評価用データセットが与えられている場合は、このステップ数が経過するたびにテストする')

                    with gr.Tab(label="Raw text file"):
                        with gr.Row():
                            raw_text_file = gr.Dropdown(choices=utils.get_datasets('training/datasets', 'txt'), value='None', label='Text file', info='トレーニングに使用する生のテキストファイル', elem_classes=['slim-dropdown'], interactive=not mu)
                        ...
                        with gr.Row():
                            with gr.Column():
                                overlap_len = gr.Slider(label='Overlap Length', minimum=0, maximum=512, value=128, step=16, info='前のチャンクのテキストから、次のチャンクにいくつのトークンを含めるか。(チャンク自体のサイズはカットオフ長で決まります）。オーバーラップをカットオフ長のちょうど半分に設定するのが理想的かもしれません')
                                newline_favor_len = gr.Slider(label='Prefer Newline Cut Length', minimum=0, maximum=512, value=128, step=16, info='オーバーラップ・カットが確実に改行でカットされるようにするための、オーバーラップ・カットをずらす最大距離の長さ（トークンではなく文字単位）。低すぎると、行の途中でカットが発生する可能性がある')

                            with gr.Column():
                                hard_cut_string = gr.Textbox(label='Hard Cut String', value='\\n\\n\\n', info='テキストパート間のハードカットを示す文字列。不要な重なりを防ぐのに役立つ')
                                min_chars = gr.Number(label='Ignore small blocks', value=0, info='ハードカットブロックの文字数がこの数より少ないか等しい場合は無視する')

                    with gr.Row():
                        start_button = gr.Button("LoRA トレーニングを開始", variant='primary', interactive=not mu)
                        stop_button = gr.Button("停止", interactive=not mu)
        ...
        with gr.Tab('Perplexity evaluation', elem_id='evaluate-tab'):
            with gr.Row():
                with gr.Column():
                    ...
                    evaluate_text_file = gr.Dropdown(choices=['wikitext', 'ptb', 'ptb_new'] + utils.get_datasets('training/datasets', 'txt')[1:], value='wikitext', label='Input dataset', info='モデルが評価される生のテキストファイル。最初のオプションは自動的にダウンロードされる：wikitext、ptb、ptb_new。次のオプションは、training/datasetsにあるローカルのテキストファイルです', interactive=not mu)
                    with gr.Row():
                        with gr.Column():
                            stride_length = gr.Slider(label='Stride', minimum=0, maximum=32768, value=512, step=256, info='正確さを犠牲にして評価を速くするために使用する。1 = 最も遅いが最も正確。512は一般的な値')

                        with gr.Column():
                            max_length = gr.Slider(label='max_length', minimum=0, maximum=shared.settings['truncation_length_max'], value=0, step=256, info='各評価のコンテキスト。0に設定すると、モデルの最大コンテキスト長が使用されます')

                    with gr.Row():
                        start_current_evaluation = gr.Button("Evaluate loaded modelロードされたモデルを評価", interactive=not mu)
                        start_evaluation = gr.Button("Evaluate selected models 選択したモデルを評価", interactive=not mu)
...
def do_train(lora_name: str, always_override: bool, q_proj_en: bool, v_proj_en: bool, k_proj_en: bool, o_proj_en: bool, gate_proj_en: bool, down_proj_en: bool, up_proj_en: bool, save_steps: int, micro_batch_size: int, batch_size: int, epochs: int, learning_rate: str, lr_scheduler_type: str, lora_rank: int, lora_alpha: int, lora_dropout: float, cutoff_len: int, dataset: str, eval_dataset: str, format: str, eval_steps: int, raw_text_file: str, overlap_len: int, newline_favor_len: int, higher_rank_limit: bool, warmup_steps: int, optimizer: str, hard_cut_string: str, train_only_after: str, stop_at_loss: float, add_eos_token: bool, min_chars: int, report_to: str):
    ...
    if model_type in MODEL_CLASSES:
        ...
        if model_type == "PeftModelForCausalLM":
            if len(shared.lora_names) > 0:
                yield "すでに別のLoRAがロードされている状態で、LoRAをトレーニングしようとしている。これは機能しますが、予期せぬ影響があるかもしれません。*(5秒後に続行されます。中断するには`中断`を押してください)*"
                logger.warning("他のLoRAの上にLoRAを重ねるトレーニング。予期せぬ効果があるかもしれない")
            else:
                yield "LoRAのロードによりモデルIDが一致しない。ベースモデルの再読み込みを検討してください。*(5秒後に続行されます。中断するには`中断`を押してください)*"
                logger.warning("LoRAのロードによりモデルIDが一致しない。ベースモデルの再ロードを検討してください")
        else:
            yield "LoRAトレーニングは現在、LLaMA、OPT、GPT-J、GPT-NeoXモデルでのみ検証されています。予期せぬエラーが発生する場合があります。*(5秒後に続行します。中断するには`中断`を押してください)*"
            logger.warning(f"LoRAトレーニングは現在、LLaMA、OPT、GPT-J、GPT-NeoXモデルでのみ検証されています(発見されたモデルタイプ: {model_type})")

        time.sleep(5)

    if shared.args.loader == 'GPTQ-for-LLaMa' and not shared.args.monkey_patch:
        yield "PTQ-for-LaLaMaを使ったLoRAのトレーニングには、`--monkey-patch`でのロードが必要です"
        return
    ...
    if raw_text_file not in ['None', '']:
        ...
        if dataset in ['None', '']:
            yield "データセットの選択入力がありません"
            return

        if format in ['None', '']:
            yield "フォーマット選択の入力がありません"
            return
    ...
    def log_train_dataset(trainer):
        ...
        try:
            ...
            logger.info("ログファイル 'train_dataset_sample.json' を 'logs' ディレクトリに作成")
        except Exception as e:
    ...
    def threaded_run():
        ...
        logger.info("LoRAトレーニングの実行が完了し、保存される")
        # Save log
    ...
    while thread.is_alive():
        time.sleep(0.5)
        if WANT_INTERRUPT:
            yield "中断します。*現在のトレーニングステップが終了すると、実行は停止します"
    ...
    if WANT_INTERRUPT:
        ...
        yield f"完了！LoRAの保存先 `{lora_file_path}`.\n\n新しいLoRAをテストする前に、まずモデルをリロードしてください"
