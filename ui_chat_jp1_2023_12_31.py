...
def create_ui():
    ...
    with gr.Tab('Chat', elem_id='chat-tab', elem_classes=("old-ui" if shared.args.chat_buttons else None)):
        ...
        with gr.Row():
            shared.gradio['mode'] = gr.Radio(choices=['chat', 'chat-instruct', 'instruct'], value='chat', label='Mode', info='チャット プロンプトの生成方法を定義します。 命令モードおよびチャット命令モードでは、[パラメータ] > [命令テンプレート] で選択した命令テンプレートが現在のモデルと一致する必要があります', elem_id='chat-mode')
...
def create_chat_settings_ui():
    ...
    with gr.Tab('Character'):
        with gr.Row():
            with gr.Column(scale=8):
                with gr.Row():
                    shared.gradio['character_menu'] = gr.Dropdown(value=None, choices=utils.get_available_characters(), label='Character', elem_id='character-menu', info='チャットおよびチャット指示モードで使用されます', elem_classes='slim-dropdown')
    ...
    with gr.Tab('Instruction template'):
        ...
        with gr.Row():
            with gr.Column():
                shared.gradio['custom_system_message'] = gr.Textbox(value=shared.settings['custom_system_message'], lines=2, label='Custom system message', info='空でない場合は、デフォルトの代わりに使用されます', elem_classes=['add_scrollbar'])
                shared.gradio['instruction_template_str'] = gr.Textbox(value='', label='Instruction template', lines=24, info='ご使用の機種/LoRAに応じて変更してください。 指示モードおよびチャット指示モードで使用されます', elem_classes=['add_scrollbar', 'monospace'])
                with gr.Row():
            ...
            with gr.Column():
                ...
                shared.gradio['chat-instruct_command'] = gr.Textbox(value=shared.settings['chat-instruct_command'], lines=4, label='Command for chat-instruct mode', info='<|character|> はボット名に置き換えられ、<|prompt|> は通常のチャット プロンプトに置き換えられます', elem_classes=['add_scrollbar'])
